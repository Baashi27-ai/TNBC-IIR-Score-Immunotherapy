...
## 2) Define gene + sample columns
gene_col    <- names(expr)[1]
sample_cols <- names(expr)[-1]

## 3) Build a fresh ENSEMBL-based table to avoid duplicates
ens_vec <- sub("\\..*$", "", expr[[gene_col]])

expr_ens <- data.table(ENSEMBL = ens_vec)
expr_ens[, (sample_cols) := expr[, ..sample_cols]]

cat("Unique Ensembl IDs:", length(unique(expr_ens$ENSEMBL)), "\n")

## 4) Map ENSEMBL → SYMBOL
ens_ids <- unique(expr_ens$ENSEMBL)
...
## 5) Merge mapping with expression (no duplicate ENSEMBL now)
merged <- merge(
  expr_ens,
  map_dt,
  by = "ENSEMBL",
...
merged <- merged[!is.na(SYMBOL)]

## 6) Aggregate by SYMBOL (average if multiple Ensembl per gene)
...
## 7) Select MHC-I / antigen presentation genes
...
## 8) Build matrix and z-score by gene
...
## 9) Per-sample MHC-I axis = mean z-score across genes
...
## 10) Restrict to TNBC-like immune OS cohort
